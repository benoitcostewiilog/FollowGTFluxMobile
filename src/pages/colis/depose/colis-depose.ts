...
import { IonicPage,NavParams, NavController, TextInput, ToastController, ViewController, Content } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-colis-depose',
  templateUrl: 'colis-depose.html',
})
export class ColisDeposePage {

  private user: UserModel = null;

  private mouvementArray: WrkMouvementModel[] = [];

  private typeIcon: string = "barcode";
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private scannerService: ScannerService, public viewCtrl: ViewController, private loginService: LoginService, private media: Media, private toastCtrl: ToastController, private barcodeScanner: BarcodeScanner, private zone: NgZone) {
  ...
  loadMouvement() {
    this.wrkMouvementDB.getProduitEnDepose(this.emplacement).then((allColis) => {
